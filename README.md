...
Полностью пример готового приложения с аутентификацией доступен по адресу [sinbadxiii/phalcon-auth-example](https://github.com/sinbadxiii/phalcon-auth-example/tree/phalcon_auth_native). Это типовой проект на Phalcon, который можно использовать как старт нового приложения, либо же просто ознакомиться с возможностями аутентификации на примере данного приложения.
...
С помощью `Phalcon\Auth\Manager` можно создать сервис провайдер аутентификации:
...
`Phalcon\Auth\Manager` по умолчанию использует конфигурацию из `$this->config->auth`, если вы хотите использовать другую конфигурацию, можно передать в качестве первого аргумента:
...
Это `Phalcon\Auth\Guard\Session` и `Phalcon\Auth\Guard\Token`. Указывая в качестве driver один из этих guards вы выбираете, что будете использовать в своем приложении, аутентификацию на основе сессий или токена,
...
Реализуя интерфейс `Phalcon\Auth\Guard\GuardInterface` вы можете создать своего Guard, добавить его в настройки и расширить список охранников `Phalcon\Auth\Manager`, например:
```php
$di->setShared('auth', function () {
    $auth = new \Phalcon\Auth\Manager();    
...
а реализация `Phalcon\Auth\RememberingInterface`:
...
Возвращаемый пользователь в виде модели `Phalcon\Auth\Adapter\User` будет реализовывать интерфейс `Phalcon\Auth\AuthenticatableInterface`, но не может использовать функцию `RememberMe` (Запомнить меня), т.к.
не имплементирует интерфейс `Phalcon\Auth\RememberingInterface` ввиду отсутствия возможности сохранить токен сессии.
...
Интерйес поставщика `Phalcon\Auth\Adapter\AdapterInterface;` имеет вид:
