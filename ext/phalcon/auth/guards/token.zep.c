...
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Auth\\Guards, Token, phalcon, auth_guards_token, phalcon_auth_guards_token_method_entry, 0);

	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("name"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("eventsManager"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("request"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("inputKey"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("storageKey"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("provider"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_guards_token_ce, SL("user"), ZEND_ACC_PROTECTED);
	zend_class_implements(phalcon_auth_guards_token_ce, 1, phalcon_auth_guards_guardinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon_Auth_Guards_Token, validate)
{
	...
	zephir_array_fetch(&_0, &credentials, &_1, PH_NOISY | PH_READONLY, "phalcon/Auth/Guards/Token.zep", 50);
	...
	zephir_array_fetch(&_4, &credentials, &_5, PH_NOISY, "phalcon/Auth/Guards/Token.zep", 54);
...
PHP_METHOD(Phalcon_Auth_Guards_Token, getTokenForRequest)
{
	...
	zephir_check_call_status();
	if (ZEPHIR_IS_EMPTY(&token)) {
		ZEPHIR_CALL_METHOD(&token, this_ptr, "bearertoken", NULL, 18);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&token);
}
...
PHP_METHOD(Phalcon_Auth_Guards_Token, bearerToken)
{
	...
	if (zephir_is_true(&_2)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", NULL, 19, &header, &_3$$3, &__$null, &_4$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_MM_RESTORE();
