...
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Auth\\Adapter\\FileProvider, Parser, phalcon, auth_adapter_fileprovider_parser, phalcon_auth_adapter_fileprovider_parser_method_entry, 0);
...
PHP_METHOD(Phalcon_Auth_Adapter_FileProvider_Parser, file)
{
	...
	if (!((zephir_file_exists(path) == SUCCESS))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_auth_exception_ce, "file dont exist", "phalcon/Auth/Adapter/FileProvider/Parser.zep", 11);
		return;
	}
	ZEPHIR_INIT_VAR(&fileData);
	zephir_file_get_contents(&fileData, path);
	object_init_ex(return_value, phalcon_auth_adapter_usercollection_ce);
	ZEPHIR_CALL_SELF(&_0, "jsonvalidate", &_1, 3, &fileData);
...
PHP_METHOD(Phalcon_Auth_Adapter_FileProvider_Parser, jsonValidate)
{
	...
	ZEPHIR_CALL_FUNCTION(&_0, "json_last_error", NULL, 4);
	zephir_check_call_status();
	do {
	...
	if (!(ZEPHIR_IS_EMPTY(&error))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$13, "__construct", NULL, 5, &error);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$13, "phalcon/Auth/Adapter/FileProvider/Parser.zep", 58);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_CCTOR(&result);
