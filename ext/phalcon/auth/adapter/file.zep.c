...
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Auth\\Adapter, File, phalcon, auth_adapter_file, phalcon_auth_adapter_file_method_entry, 0);

	zend_declare_property_null(phalcon_auth_adapter_file_ce, SL("collection"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_adapter_file_ce, SL("hasher"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_adapter_file_ce, SL("config"), ZEND_ACC_PROTECTED);
	zend_class_implements(phalcon_auth_adapter_file_ce, 1, phalcon_auth_adapter_adapterinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon_Auth_Adapter_File, __construct)
{
	...
	ZEPHIR_CALL_CE_STATIC(&_0, phalcon_auth_adapter_fileprovider_parser_ce, "file", &_1, 0, &_2);
...
PHP_METHOD(Phalcon_Auth_Adapter_File, retrieveById)
{
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("collection"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_OBS_VAR(&data);
	zephir_array_fetch(&data, &_0, identifier, PH_NOISY, "phalcon/Auth/Adapter/File.zep", 27);
	ZEPHIR_INIT_VAR(&_1);
	if (zephir_is_true(&data)) {
		object_init_ex(&_1, phalcon_auth_adapter_user_ce);
		ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 1, &data);
		zephir_check_call_status();
	} else {
		ZVAL_NULL(&_1);
	}
...
PHP_METHOD(Phalcon_Auth_Adapter_File, validateCredentials)
{
	...
	if (zephir_is_true(&_2)) {
		...
		zephir_array_fetch_string(&_4, &credentials, SL("password"), PH_NOISY | PH_READONLY, "phalcon/Auth/Adapter/File.zep", 34);
		ZEPHIR_CALL_METHOD(&_5, user, "getauthpassword", NULL, 0);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_0, &_3, "checkhash", NULL, 0, &_4, &_5);
		zephir_check_call_status();
	} else {
		zephir_array_fetch_string(&_6, &credentials, SL("password"), PH_NOISY | PH_READONLY, "phalcon/Auth/Adapter/File.zep", 35);
