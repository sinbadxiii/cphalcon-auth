...
{
	ZEPHIR_REGISTER_CLASS_EX(Phalcon\\Auth\\Access, AccessAbstract, phalcon, auth_access_accessabstract, zephir_get_internal_ce(SL("phalcon\\di\\injectable")), phalcon_auth_access_accessabstract_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	zend_declare_property_null(phalcon_auth_access_accessabstract_ce, SL("exceptActions"), ZEND_ACC_PROTECTED);
	zend_declare_property_null(phalcon_auth_access_accessabstract_ce, SL("onlyActions"), ZEND_ACC_PROTECTED);
	phalcon_auth_access_accessabstract_ce->create_object = zephir_init_properties_Phalcon_Auth_Access_AccessAbstract;

	zend_class_implements(phalcon_auth_access_accessabstract_ce, 1, phalcon_auth_access_accessinterface_ce);
	return SUCCESS;
}
