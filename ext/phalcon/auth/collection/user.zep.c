...
{
	ZEPHIR_REGISTER_CLASS(Phalcon\\Auth\\Collection, User, phalcon, auth_collection_user, phalcon_auth_collection_user_method_entry, 0);

	zend_declare_property_null(phalcon_auth_collection_user_ce, SL("id"), ZEND_ACC_PRIVATE);
	zend_declare_property_null(phalcon_auth_collection_user_ce, SL("key"), ZEND_ACC_PRIVATE);
	zend_declare_property_null(phalcon_auth_collection_user_ce, SL("password"), ZEND_ACC_PRIVATE);
	zend_class_implements(phalcon_auth_collection_user_ce, 1, phalcon_auth_authenticatableinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon_Auth_Collection_User, __construct)
{
	...
	zephir_is_iterable(data, 0, "phalcon/Auth/Collection/User.zep", 18);
