...
ZEPHIR_INIT_CLASS(Phalcon_Auth_Collection_UsersCollection)
{
	ZEPHIR_REGISTER_CLASS_EX(Phalcon\\Auth\\Collection, UsersCollection, phalcon, auth_collection_userscollection, zephir_get_internal_ce(SL("phalcon\\support\\collection")), phalcon_auth_collection_userscollection_method_entry, 0);

	zend_declare_property_null(phalcon_auth_collection_userscollection_ce, SL("collection"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);
	return SUCCESS;
}

PHP_METHOD(Phalcon_Auth_Collection_UsersCollection, __construct)
{
	...
	zephir_update_static_property_ce(phalcon_auth_collection_userscollection_ce, ZEND_STRL("collection"), &data);
	if (insensitive) {
		ZVAL_BOOL(&_0, 1);
	} else {
		ZVAL_BOOL(&_0, 0);
	}
	ZEPHIR_CALL_PARENT(NULL, phalcon_auth_collection_userscollection_ce, getThis(), "__construct", NULL, 0, &data, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
}

PHP_METHOD(Phalcon_Auth_Collection_UsersCollection, find)
{
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	...
	if (Z_TYPE_P(&keys) != IS_NULL) {
		...
		if (Z_TYPE_P(&keys) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&keys), _3$$4)
			{
				...
				zephir_read_static_property_ce(&_5$$5, phalcon_auth_collection_userscollection_ce, SL("collection"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_OBS_NVAR(&data$$5);
				zephir_array_fetch(&data$$5, &_5$$5, &key$$4, PH_NOISY, "phalcon/Auth/Collection/UsersCollection.zep", 30);
			...
			while (1) {
				...
				zephir_check_call_status();
					zephir_read_static_property_ce(&_8$$6, phalcon_auth_collection_userscollection_ce, SL("collection"), PH_NOISY_CC | PH_READONLY);
					ZEPHIR_OBS_NVAR(&data$$6);
					zephir_array_fetch(&data$$6, &_8$$6, &key$$4, PH_NOISY, "phalcon/Auth/Collection/UsersCollection.zep", 30);
...
PHP_METHOD(Phalcon_Auth_Collection_UsersCollection, first)
{
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
